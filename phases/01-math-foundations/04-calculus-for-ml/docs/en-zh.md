...
![f(x,y) = x^2 + y^2 的等高线图](assets/contour-plot.svg)

该函数呈碗状，等值线为同心圆。最小值在 $(0, 0)$ 处。$(1, 1)$ 处的蓝色箭头指向负梯度方向，即下坡朝向最小值的方向。
...
初学者常问：*"梯度不就是误差的方向吗？为什么不直接减去误差，还要乘个学习率？"*

关键区别在于：**梯度告诉你方向和多陡，但不告诉你应该走多远。**

用一个具体例子说明。假设 $f(x) = x^2$，当前在 $x=5$：

$$
x = 5, \quad f(5) = 25, \quad f'(5) = 10
$$

如果去掉学习率（即认为 $\text{lr} = 1$）：

$$
x_{\text{new}} = 5 - 1 \times 10 = -5
$$

一步就从 5 跃到了 -5，损失从 25 变成 25——**没有任何改善**。如果起点在 $x=10$（梯度 = 20），一步就飞到 -10，彻底发散。**梯度的绝对值取决于函数当前的陡峭程度，和"到最小值还有多远"没有直接关系。**

再看为什么不能直接用"误差"替代"梯度"。在线性回归 $y = wx + b$ 中：

$$
\frac{\partial L}{\partial w} = \frac{2}{n} \sum_{i=1}^{n} (\underbrace{wx_i + b - y_i}_{\text{误差}}) \cdot x_i
$$

梯度 = 误差 × 输入值 $x$。直接减误差会忽略 $x$ 的影响：
- 如果 $x = 1000$，即使误差很小，**权重微调也会大幅改变输出**——需要小步
- 如果 $x = 0.001$，即使误差很大，**权重需要大步调整才能产生可见的变化**

学习率将"方向信息"（梯度）和"步长控制"（学习率）解耦，让优化器独立调节二者。

从泰勒视角看，梯度下降基于一阶近似 $L(w + \Delta w) \approx L(w) + L'(w) \cdot \Delta w$，这个近似只在当前点附近成立。学习率确保步子不会迈出线性近似的有效范围。

**具体到实际场景：**

| 场景 | 梯度大小 | 合适步长 | 无学习率的结果 |
|------|---------|---------|--------------|
| 陡峭峡谷 | 很大 | 小步（0.01） | 发散爆炸 |
| 平缓高原 | 很小 | 大步（0.1） | 几乎不动 |
| 震荡区域 | 忽大忽小 | 中等自适应 | 来回跳跃 |
